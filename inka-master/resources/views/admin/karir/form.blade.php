@slot('body')
	<div class="form-group">
		{!! Form::label('nama', 'Nama Karir', array('class' => 'required-input')) !!}
		{!! Form::text('nama', null, array('placeholder' => 'Nama Karir','class' => 'form-control')) !!}
	...
	<div class="form-group">
		...
		{!! Form::date('selesai', null, array('placeholder' => 'Tanggal Selesai','class' => 'form-control')) !!}
		...
		<div class="document-list">
			@if(isset($edit))
				@foreach($karir->document as $item)
					<div class="row karir-file">
						<div class="col-xs-10">
							<a href="{{route('karir.download-document', ['id' => $item->id])}}" target="_blank" class="form-control">{{$item->nama}}</a>
						</div>
						<div class="col-xs-2">
...
@slot('footer')
<div class="form-group">
  {!! link_to_route('karir.index', 'Kembali', null, array('class' => 'btn  btn-default')) !!}
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('.delete-document').on('click', function(){
				...
				$.ajax({
			        method: 'POST',
			        url: '{{route('karir.delete-document')}}',
			        dataType: 'json',
			        data: {
